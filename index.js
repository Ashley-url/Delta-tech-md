...
const fs = require('fs');
const fsSync = require('fs').promises; // Added for async file operations
...
const chalk = require('chalk'); // Added for colored logs
...
async function loadSession() {
  try {
    ...
    if (config.SESSION_ID.startsWith("starcore~")) {
      ...
      // Decode base64 to buffer and write to creds.json
      ...
      await fsSync.writeFile(credsPath, decodedData);
      console.log(chalk.green("[ ✅ ] Base64 session decoded and saved successfully"));
      return sessionData;
    } 
    // Handle MEGA.nz session with suho~ prefix
      ...
      await fsSync.writeFile(credsPath, data);
      console.log(chalk.green("[ ✅ ] MEGA session downloaded successfully"));
      return JSON.parse(data.toString());
    } else {
      throw new Error("Invalid SESSION_ID format. Use 'deltacore~' for base64 or 'suho~' for MEGA.nz");
    }
  } catch (error) {
...
setTimeout(() => {
  connectToWA();
}, 4000);  const StickersTypes = require('wa-sticker-formatter')
  const util = require('util')
  const { sms, downloadMediaMessage, AntiDelete } = require('./lib')
  const FileType = require('file-type');
  const axios = require('axios')
